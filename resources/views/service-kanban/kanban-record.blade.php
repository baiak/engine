...
>
    ...
    font-size: 14px;
"> <!-- IDEIA: Aqui vamos obter: $record->order->order_number
        vamos buscar os servicos filtrados com o order_number
        vamos verificar os status dos servicos, caso tenha mais de um serviço com o mesmo status
        crie uma div com o order_number como titulo, e um botao dropdown ira exibir os cards com todos os serviços relacionados com
        o numero da ordem
!-->
        ...
        color: #333;
    ">
            Ordem #{{$record->order->order_number}}
        </h3>
        <p style="margin: 8px 0; color: #555;">
            <strong>Cliente:</strong> {{$record->order->client->name}}
        </p>
        <p style="margin: 8px 0; color: #555;">
            <strong>Peça:</strong> {{$record->part->title}}
        </p>
        <p style="margin: 8px 0; color: #555;">
            <strong>Veículo:</strong> {{$record->order->vehicle->factory}} / {{$record->order->vehicle->model}} / {{$record->order->vehicle->motor}}
        </p>
